...
V=./nucleo/target/release/vbl
RAPL=/sys/devices/virtual/powercap/intel-rapl/intel-rapl:0/energy_uj
CFG=logs/etapa5/lab/fxp-lab.cfg
VL=logs/etapa5/lab/lab-energia.vl
E() { cat "$RAPL"; }
...
for i in 1 2 3; do
  ...
  e0=$(E)
  OUT=$($V run "$VL" --ticks 92 --real-ms 1000 --fxp-config "$CFG" --caderno "logs/etapa5/lab/rapl-precisao-$i.vcad" 2>&1)
  e1=$(E)
  ECAD=$(echo "$OUT" | J)
