...
|---|---|---|
| 4.1 Joules por forma (partilha P/N × tick, FORMAL §4.2) | ✅ (já na Etapa 2) + agregados de produção | `production_ledger.rs` (`Resumo`: total, por forma, média) |
| 4.1 Registro de I/O: leitura (valor, timestamp) e atuação (ator, solicitado, aplicado, latência, custo) | ✅ | `ledger.rs` (`Actuation`, `actuator_action_detailed`), `vbl-fxp/src/bus.rs` (latência medida com `Instant`), `sim.rs` (fronteira em processo) |
| 4.1 Gravação assíncrona (buffer + flush periódico) | ✅ | `production_ledger.rs` (thread + canal, BufWriter 64 KiB, flush a cada 256 eventos) |
...
7.0.0-29. Benches criterion (`cargo bench --bench notebook`, perfil bench =
...
| **Logs íntegros verificados** | Cadeia SHA-256 recomputada do arquivo em toda execução com `--ledger` + `ledger-verify` nos 3 logs versionados + estresse 60.000 eventos — ÍNTEGRA; JSONL adulterado → exit 1 | ✅ |
...
| Corrupção (auditoria) | JSONL adulterado → `ledger-verify` exit 1, "CORROMPIDA" |
...
```mermaid
flowchart LR
    RT["Engine de tick<br/>(genérico no Caderno)"] -->|"record/leak/actuação"| CP["ProductionLedger<br/>seq + relógio + agregados"]
    ...
    VCAD --> CV["vbl ledger-verify<br/>(agente externo)"]
    JSONL --> CV
...
- **Engine genérico:** `Engine<F: Fxp, C: Caderno = ChainLedger>` — o runtime
  não mudou de semântica (default = implementação de referência); o CLI injeta
  o Caderno de produção com `--ledger`.
- **CLI:** `vbl run --ledger ARQUIVO` (binário em `ARQUIVO` + JSONL em
  `ARQUIVO.jsonl`, verificação embutida ao final, exit 1 se corrompido);
  `vbl ledger-verify ARQUIVO`; flags de injeção de falha para os cenários
  (`--fail-actor`, `--fallback PRIM=ALT`, `--register-actor`).
...
core/target/release/vbl run estresse10k.vl --ticks 5 --ledger stress-10k.vcad
core/target/release/vbl ledger-verify stress-10k.vcad
```
