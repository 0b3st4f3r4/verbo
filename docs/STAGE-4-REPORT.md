...
|---|---|---|
| 4.1 Joules por forma (partilha P/N × tick, FORMAL §4.2) | ✅ (já na Etapa 2) + agregados de produção | `production_notebook.rs` (`Resumo`: total, por forma, média) |
| 4.1 Registro de I/O: leitura (valor, timestamp) e atuação (ator, solicitado, aplicado, latência, custo) | ✅ | `notebook.rs` (`Atuacao`, `actuator_action_detalhada`), `vbl-fxp/src/bus.rs` (latência medida com `Instant`), `sim.rs` (fronteira em processo) |
| 4.1 Gravação assíncrona (buffer + flush periódico) | ✅ | `production_notebook.rs` (thread + canal, BufWriter 64 KiB, flush a cada 256 eventos) |
...
| Formato binário compacto documentado | ✅ | [`docs/NOTEBOOK-FORMAT-v1.md`](NOTEBOOK-FORMAT-v1.md) + codec em `production_notebook.rs` |
...
| `actor-failure-fallback.vcad` (+`.jsonl`) | BDD Caso 3 (`--falhar-ator Ventoinha`) | 29 | 600,00 J | ÍNTEGRA |
| [`VERIFICATION.txt`](../logs/stage4/VERIFICATION.txt) | saída do `vbl caderno-verify` sobre os três + estresse 60k | — | — | ÍNTEGRA |
...
core/target/release/vbl run estresse10k.vl --ticks 5 --caderno stress-10k.vcad
core/target/release/vbl caderno-verify stress-10k.vcad
```
