...
| 5.2 Validação final de integridade (AD) | ✅ | Checklist do AGENTS §1.1 aplicado (§7) + revisão de metas ([ETAPA-5-METAS-REVISAO.md](ETAPA-5-METAS-REVISAO.md)) |
| 5.3 Compilador final com relatórios de performance | ✅ | Este relatório + `logs/etapa5/` (baselines criterion antes/depois, soak, ASan, logs `.vcad` verificados) |
...
| **Consumo de memória dentro dos limites** | Steady-state 10.000 formas: **7,43 MB ≤ 10 MB** ✅; por forma: `event` **743 B**, `equilibrium` **743 B**, `nonequilibrium`+1 regra **1 448 B** — os orçamentos provisórios (256 B/1 KB/512 B) não cabem em contêineres std e foram **reancorados** (§4; [METAS](ETAPA-5-METAS-REVISAO.md)) | ✅ com revisão formal |
...
saída completa em [`logs/etapa5/bench-depois-vs-antes.txt`](../logs/etapa5/bench-depois-vs-antes.txt).
...
(`tests/caderno_producao.rs`): referência independente via `ChainCaderno`,
...
[`docs/ETAPA-5-METAS-REVISAO.md`](ETAPA-5-METAS-REVISAO.md) (AGENTS §4), com
...
- **Soak de 15 min (~240 mil ticks, 666 formas vivas renovadas a cada 3
  ...
  `SOAK OK`. Log completo: [`logs/etapa5/soak-15min.txt`](../logs/etapa5/soak-15min.txt).
...
[METAS-REVISAO §5](ETAPA-5-METAS-REVISAO.md). No CI/sessão, a contabilidade
...
## 8. Logs e artefatos ([`logs/etapa5/`](../logs/etapa5/))

| Arquivo | Conteúdo |
|---|---|
| `baseline/bench-antes.txt` | criterion `--save-baseline antes` (pré-otimização) |
| `bench-depois-vs-antes.txt` | criterion `--baseline antes` (deltas por bench) |
| `soak-15min.txt` | soak de longa execução (RSS/platenga, `SOAK OK`) |
| `asan-resumo.txt` | suíte completa sob ASan/LSan — limpa |
| `logs/subversao-termica.vcad` (+`.jsonl`) | BDD Caso 2 pós-otimização — cadeia ÍNTEGRA |
| `logs/fadiga-atencao.vcad` (+`.jsonl`) | BDD Caso 1 — cadeia ÍNTEGRA |
| `logs/tarefa-main.vcad` (+`.jsonl`) | bloco `main`/keep — cadeia ÍNTEGRA |
| `logs/estresse-10k.vcad` (+`.jsonl`) | 10.000 formas × 5 ticks = **60.012 eventos**, 750,00 J — cadeia ÍNTEGRA (`caderno-verify`) |
...
cd nucleo && cargo bench --bench caderno -- --baseline antes   # deltas
...
1. **Dissolução O(1) amortizada** (tombstones + compaction) — proposta com
   análise de risco em [METAS-REVISAO §1](ETAPA-5-METAS-REVISAO.md).
