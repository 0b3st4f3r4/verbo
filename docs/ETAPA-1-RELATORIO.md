# ETAPA-1-RELATORIO.md — Pesquisa & Arquitetura de Escopo via Código (TDD/BDD/E2E)
...
| §1.3 c — banco de 20 prompts | `docs/CHEATSHEET-PROMPTS.yaml` (S01–S10 sintaxe, M01–M10 semântica) + `scripts/validate_cheatsheet.py` (3 execuções por prompt, verificador `tests/vlcheck.py`, relatório versionado em `docs/CHEATSHEET-VALIDACAO.md`, limiar ≥ 90 %) |
...
|---|---|---|
| Sensor ausente | FORMAL §4.7 | `test_falha_sensores.py::test_sensor_ausente_nao_avalia_condicao_nem_dispara`, `::test_sensor_ausente_nao_e_tratado_como_zero`; `test_clausulas_erro.py::test_sensor_nao_registrado_detectado_no_ir` |
| Ator inexistente | FORMAL §4.3 | `test_atores.py::test_ator_inexistente_rejeitado_com_registro`; `test_clausulas_erro.py::test_ator_nao_registrado_detectado_no_ir` |
| Valor fora de limite | FORMAL §4.3 | `test_atores.py::test_valor_abaixo_do_minimo_rejeitado_sem_envio`, `::test_valor_acima_do_safety_limit_rejeitado`, `::test_valores_fora_de_limite_rejeitados[*]`, `::test_rejeicao_nao_dissolve_a_forma` |
| Forma sem `value`/`horizon` | FORMAL §3 · Lei 1 | `test_clausulas_erro.py::test_forma_sem_value_rejeitada_no_texto`, `::test_forma_sem_horizon_rejeitada_no_texto`, `::test_value_antes_de_horizon_e_exigido_no_texto`, `::test_forma_sem_value_ou_horizon_rejeitada_no_ir` |
| Review órfã/duplicada | FORMAL §3 | `test_clausulas_erro.py::test_review_orfa_rejeitada_no_texto`, `::test_review_duplicada_rejeitada_no_texto`, `::test_review_orfa_rejeitada_no_ir`, `::test_review_duplicada_rejeitada_no_ir` |
| `keep` de forma inexistente | AGENTS §2.2 | `test_clausulas_erro.py::test_keep_de_forma_inexistente_rejeitado_no_texto`, `::test_keep_de_forma_inexistente_rejeitado_no_ir`, `::test_keep_de_forma_dissolvida_registrado_em_runtime` |
...
| `reclassify_as_nonequilibrium` sem deadline declarado | `test_clausulas_erro.py::test_reclassify_para_nonequilibrium_sem_deadline_declarado` (e variante legal `::..._com_deadline_declarado`) |
| Ação sobre forma já dissolvida → `review_after_dissolution` | `test_clausulas_erro.py::test_keep_de_forma_dissolvida_registrado_em_runtime` (guarda do runtime; evento testado via contrato do tick) |
...
| Semântica | Teste |
|---|---|
| Mensagem FXP serializada e entregue ao ator correto | `test_atores.py::test_act_e_serializado_e_entregue_ao_ator_correto`; BDD Caso 2 |
| Limites inclusivos (min/max/safety) | `test_atores.py::test_limites_sao_inclusivos` |
...
| Fallback: política do registro do FXP + trilha no Caderno | BDD Caso 3; `test_atores.py::test_fallback_executado_quando_primario_nao_responde`, `::test_fallback_esgotado_registra_alerta` |
...
`test_falha_sensores.py` (6 testes): `0.0` é leitura válida que dispara
...
| (i) `0.0` válido | `test_falha_sensores.py::test_leitura_zero_e_valida_e_dispara_regras` |
| (ii) sensor ausente/inacessível sem disparo falso | `test_falha_sensores.py` |
...
| Comandos `act` serializados e entregues (mock) | `test_atores.py`; BDD Casos 2–3 |
...
make validate-cheatsheet   # 20 prompts × 3 execuções → docs/CHEATSHEET-VALIDACAO.md
```
...
[`docs/CHEATSHEET-VALIDACAO.md`](CHEATSHEET-VALIDACAO.md); respostas brutas
