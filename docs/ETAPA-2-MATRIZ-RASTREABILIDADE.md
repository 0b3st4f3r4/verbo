...
das notas semânticas com ≥ 1 teste. Suíte: `nucleo/crates/vbl-lang/tests/matriz_producoes.rs`
(42 testes, todos passando) + `nucleo/crates/vbl-lang/tests/canon_roundtrip.rs` (5).
...
| 9 | unidade do `threshold` é capturada na AST para validação contra o registro do FXP (validada em runtime — `unidade_ausente`/`unidade_incompativel`, testes em `vbl-runtime/tests/transicao.rs` via loader) | `nota_threshold_unidade_e_capturada_para_validacao_no_registro` | ✅ |
...
| `keep_forma_inexistente` | `producao_main_statement_desconhecido_e_keep_inexistente` (parser) + `keep_forma_inexistente` em runtime (`vbl-runtime/tests/transicao.rs` via `main`) |
...
(`vbl-runtime/tests/transicao.rs`):

| Código | Teste(s) em `transicao.rs` |
|--------|----------------------------|
...
estão em `exemplos/*.vl` e são executados ponta a ponta pelo CLI `vbl`
(`vbl check`, `vbl run`).
