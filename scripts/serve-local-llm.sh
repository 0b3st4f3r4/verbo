...
if [[ "${LOCAL_LLM_UI:-1}" != "0" ]]; then
  ...
  UI_URL="http://127.0.0.1:${UI_PORT}/scripts/verbo-chat/chat.html#k=${KEY}&u=http%3A%2F%2F127.0.0.1%3A8000%2Fv1&m=${SERVED}&c=${CTX}"
  (
    http_pid=""
    for _ in $(seq 1 120); do                    # ~10 min de paciência
      ...
      if curl -fsS -H "Authorization: Bearer ${KEY}" http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
        if ! curl -fsS "http://127.0.0.1:${UI_PORT}/scripts/verbo-chat/chat.html" >/dev/null 2>&1; then
