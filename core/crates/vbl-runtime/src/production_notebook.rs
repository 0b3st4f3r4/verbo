...
//! — ver [`crate::notebook::Evento::linha`]); o hash do frame é o elo da cadeia
//! em bytes crus. O verificador recomputa `hash_n = SHA-256(hash_{n-1} ||
//! linha_n)` — adulteração retroativa quebra a cadeia.

use crate::notebook::{carimbar_tempo, sha256_hex_duplo, Caderno, ChainCaderno};
    ...
    while let Ok(msg) = rec.recv() {
        match msg {
            ...
            Msg::Evento { seq, tick, t, kind, msg, mut extra } => {
                ...
                let evento =
                    crate::notebook::Evento { seq, kind, msg, extra, hash: String::new() };
                linha.clear();
        ...
        let digest = crate::notebook::sha256_bytes_duplo(head.as_bytes(), linha.as_bytes());
        w.write_all(&(linha.len() as u32).to_le_bytes())?;
        w.write_all(linha.as_bytes())?;
        w.write_all(&digest)?;
        head.clear();
        crate::notebook::escrever_hex(&digest, &mut head);
        eventos += 1;
...
/// Equivalência garantida por teste (`tests/production_notebook.rs`).
