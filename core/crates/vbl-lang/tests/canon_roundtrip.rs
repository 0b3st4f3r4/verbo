...
use vbl_lang::{
    canon::form_to_vl, parse, Conjugation, Duration, ExprKind, Expression, FormAttrs, FormDecl,
...
fn example_form(conjugation: Conjugation) -> FormDecl {
    FormDecl {
        ...
        name: "Exemplo".into(),
        value: Expression {
            kind: ExprKind::Str("conteúdo \"com\" escapes".into()),
        ...
        attrs: FormAttrs {
            ...
            maintenance_deadline: Some(Duration {
                value: 3.0,
                unit: TimeUnit::S,
                span: Default::default(),
            }),
...
fn roundtrip(f: &FormDecl) -> FormDecl {
    ...
    assert!(
        !diags.has_errors(),
        "`.vl` canônico não reparseou:\n{text}\n{diags}"
    );
    let reparseada = program.forms().next().unwrap().clone();
    assert_eq!(
        form_to_vl(&reparseada),
        text,
        "serialização não é ponto fixo"
    );
    reparseada
}
...
fn canonical_equilibrium_without_optionals_is_minimal() {
    let f = FormDecl {
        ...
        name: "Piscada".into(),
        value: Expression {
            kind: ExprKind::Str("impulso_curto".into()),
            span: Default::default(),
        },
        horizon: Duration {
            value: 2.0,
